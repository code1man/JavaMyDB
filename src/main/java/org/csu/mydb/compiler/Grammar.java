...
//        System.out.println("=== FIRST 集 ===");
//        for (NonTerminal nt : nonTerminals) {
//            System.out.print(nt + ": { ");
//            Set<Terminal> s = firstMap.get(nt);
//            if (s != null) for (Terminal t : s) System.out.print(t + " ");
//            System.out.println("}");
//        }
//    }
//
 ...
 * @author ljyljy
 * @date 2025/9/12
 * @description 文法（支持 QualifiedName 与 简单 JOIN ... ON ...）
 */
public class Grammar {
    // 非终结符
    public static class NonTerminal {
    ...
    public Grammar() {
        ...
        // 新增用于 JOIN 的非终结符
        NonTerminal QualifiedName = new NonTerminal("QualifiedName"); // IDENTIFIER | IDENTIFIER . IDENTIFIER
        NonTerminal TableRef = new NonTerminal("TableRef"); // IDENTIFIER ( IDENTIFIER )?
        NonTerminal JoinOpt = new NonTerminal("JoinOpt"); // ε | JOIN TableRef ON QualifiedName = QualifiedName
        ...
        nonTerminals.addAll(Arrays.asList(
                ...
                AlterStmt, AlterAction,
                InsertTail, DropTail, AssignListTail,
                QualifiedName, TableRef, JoinOpt, JoinClause
        ));

        // ---------------- 终结符（确保包含 EVERYTHING、JOIN、ON、.） ----------------
        String[] kw = {"SELECT","FROM","WHERE","CREATE","TABLE","INSERT","INTO","VALUES",
                "UPDATE","SET","DELETE","DROP","DATABASE","USE",
                "AND","NOT","PRIMARY","KEY","INT","VARCHAR","CHAR","EVERYTHING","JOIN","ON","NULL"};
        // 增加 "." 为符号 (用于限定名), "=" 在 symbols 列表里也存在
        String[] symbols = {",",";","(",")",".","=","!=","<>",">","<",">=","<="};
        for (String s : kw) terminals.add(new Terminal(s));
        for (String s : symbols) terminals.add(new Terminal(s));
        // 基础终结符
        ...
        // ---------------- Select ----------------
        // SelectStmt -> SELECT ColumnList FROM TableRef JoinOpt WhereClause ;
        addProduction(SelectStmt, Arrays.asList(new Terminal("SELECT"), ColumnList, new Terminal("FROM"), TableRef, JoinOpt, WhereClause, new Terminal(";")));

        // ColumnList -> EVERYTHING | QualifiedName ColumnListTail
        addProduction(ColumnList, Arrays.asList(new Terminal("EVERYTHING")));
        addProduction(ColumnList, Arrays.asList(QualifiedName, ColumnListTail));
        // ColumnListTail -> , QualifiedName ColumnListTail | ε
        addProduction(ColumnListTail, Arrays.asList(new Terminal(","), QualifiedName, ColumnListTail));
        addProduction(ColumnListTail, Arrays.asList(EPSILON));

        // QualifiedName -> IDENTIFIER '.' IDENTIFIER | IDENTIFIER
        addProduction(QualifiedName, Arrays.asList(new Terminal("IDENTIFIER"), new Terminal("."), new Terminal("IDENTIFIER")));
        addProduction(QualifiedName, Arrays.asList(new Terminal("IDENTIFIER")));

        // TableRef -> IDENTIFIER ( IDENTIFIER )?   // 表名 可选 别名
        addProduction(TableRef, Arrays.asList(new Terminal("IDENTIFIER")));
        addProduction(TableRef, Arrays.asList(new Terminal("IDENTIFIER"), new Terminal("IDENTIFIER")));

        // JoinOpt -> ε | JOIN TableRef ON QualifiedName = QualifiedName
        addProduction(JoinOpt, Arrays.asList(EPSILON));
        addProduction(JoinOpt, Arrays.asList(new Terminal("JOIN"), TableRef, new Terminal("ON"), QualifiedName, new Terminal("="), QualifiedName));
        ...
        // Condition -> ConditionTerm ConditionTail
        addProduction(Condition, Arrays.asList(ConditionTerm, ConditionTail));
        // ConditionTail -> AND Condition | ε      (只支持 AND 链接)
        addProduction(ConditionTail, Arrays.asList(new Terminal("AND"), Condition));
        addProduction(ConditionTail, Arrays.asList(EPSILON));

        // ConditionTerm -> IDENTIFIER Operator Value
        // (这里仍然使用 IDENTIFIER：如果需要支持 QualifiedName 在 WHERE 左侧，可替换为 QualifiedName)
        // 为了兼容可能出现的 qualified 在条件中（如 A.col = B.col），我们也加入支持：
        addProduction(ConditionTerm, Arrays.asList(new Terminal("IDENTIFIER"), Operator, Value));
        // 可选地支持 qualified 左侧或右侧（你可以在 parser/PlanBuilder 层做增强）
