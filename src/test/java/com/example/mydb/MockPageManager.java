...
public class MockPageManager {
    ...
    private static final Map<Integer, ConcurrentHashMap<Integer, PageManager.Page>> fakeDisk = new ConcurrentHashMap<>();
    private static final AtomicInteger nextPageNo = new AtomicInteger(0);

    /**
     * 创建一个新的 DataPage 并写入一条数据
     */
    public static PageManager.Page newPage(PageManager.GlobalPageId pid, String data) {
        fakeDisk.putIfAbsent(pid.spaceId, new ConcurrentHashMap<>());
        PageManager.Page page = new PageManager.Page(pid.pageNo);
     ...
     * 模拟写入磁盘
     */
    public static void writePage(PageManager.Page page, int spaceId) throws IOException {
     ...
     * 模拟从磁盘读取
     */
    public static PageManager.Page readPage(int spaceId, int pageNo) throws IOException {
        if (!fakeDisk.containsKey(spaceId)) {
            throw new IOException("Table (spaceId) not found: " + spaceId);
        }
        PageManager.Page page = fakeDisk.get(spaceId).get(pageNo);
        if (page == null) {
