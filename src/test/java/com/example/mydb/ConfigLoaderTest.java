...
public class ConfigLoaderTest {
    ...
    public static void setup() throws Exception {
        ConfigLoader.getInstance().loadConfig("config/mydb_test.ini");
    }

    // 测试 1：配置文件加载是否成功（无异常即成功）
    @Test
    public void testLoadConfig_Success() {
        assertDoesNotThrow(() -> ConfigLoader.getInstance().loadConfig("config/mydb_test.ini"));
    }

    // 测试 2：获取存在的字符串配置项
    @Test
    public void testGetString_ExistingKey() {
        String logLevel = ConfigLoader.getInstance().getString("log", "level", "INFO");
    ...
    public void testGetString_MissingKey() {
        String nonExistentKey = ConfigLoader.getInstance().getString("log", "non_existent", "DEFAULT");
    ...
    public void testGetInt_ExistingKey() {
        int pageSize = ConfigLoader.getInstance().getInt("storage", "page_size", 4096);
    ...
    public void testGetInt_InvalidFormat() {
        int invalidInt = ConfigLoader.getInstance().getInt("storage", "invalid_int", 4096);
    ...
    public void testGetBool_ExistingKey() {
        boolean cacheEnabled = ConfigLoader.getInstance().getBool("log", "enabled", false);
    ...
    public void testGetBool_InvalidFormat() {
        boolean invalidBool = ConfigLoader.getInstance().getBool("log", "path", false);  // path 是字符串，非布尔
    ...
    public void testGetString_MissingSection() {
        String missingSectionKey = ConfigLoader.getInstance().getString("missing_section", "any_key", "DEFAULT");
    ...
    public void testGetString_EmptyValue() {
        String emptyValueKey = ConfigLoader.getInstance().getString("existing_section", "missing_key", "DEFAULT");
    ...
    public void testThreadSafety() throws InterruptedException {
        ...
        for (int i = 0; i < threadCount; i++) {
            threads[i] = new Thread(() -> {
                for (int j = 0; j < 100; j++) {
                    String value = ConfigLoader.getInstance().getString("log", "level", "INFO");
